    ...
    stage('Publish Pacts') {
      steps {
        //-- set prod if want to deploy prod tag
         if(parameters.pactConsumerTags) {
            sh 'mvn pact:publish -Dpactbroker.url=http://pact_broker:80 -Dpact.consumer.version=${GIT_COMMIT} -Dpact.tag=${parameters.pactConsumerTags}'
        }
      }
    }
